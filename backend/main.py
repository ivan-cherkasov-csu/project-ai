...
if __name__ == "__main__":
    ...
    query = 'Hey can you please find in local storage a project with the Salad recipe?'   
    ...
    print("--------------------MODEL_END--------------------")
    query = "Please fix Salad project from storage by adding relevant data, create task for each step in recipe, and add required resources. You can use web search to get data from the internet if you need."
    result = chat_agent.run(ChartQuery(query=query))
    print(result.answer)
    print("--------------------MODEL_START--------------------")
    print(result.model_dump_json())
